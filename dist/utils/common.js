...
exports.updateNestCli = updateNestCli;
...
async function updateNestCli(projectRoot, newConfigs) {
    if (!newConfigs.length)
        return;
    const filePath = path.join(projectRoot, "nest-cli.json");
    const nestCliConfig = await fs.readJson(filePath);
